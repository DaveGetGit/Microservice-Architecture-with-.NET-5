...
namespace Catalog.API.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            FilterDefinition<Product> filterDefinition = Builders<Product>.Filter.Eq(a => a.Category, categoryName);
            return await _context.Products.Find(filterDefinition).ToListAsync();
        }

        public async Task<IEnumerable<Product>> GetProductByName(string name)
        {
            FilterDefinition<Product> filterDefinition = Builders<Product>.Filter.Eq(a => a.Name, name);
            return await _context.Products.Find(filterDefinition).ToListAsync();
        }
